...
var getAccessToken = async () => {
  ...
  const response = await import_axios.default.post("https://arvesta-dev.apimanagement.hana.ondemand.com/v1/OAuthService/GenerateToken", params.toString());
  return response.data.access_token;
};
var handler = async (event) => {
  const { email } = JSON.parse(event.body);
  const website = event.path !== "/" ? event.rawUrl.split(event.path)[0] : event.rawUrl;
  try {
    const accessToken = await getAccessToken();
    console.log("\u{1F680} ~ file: subscribe.ts ~ line 55 ~ consthandler:Handler= ~ accessToken", accessToken);
    const body = `
    ...
    const response = await import_axios.default.post("https://arvesta-dev.apimanagement.hana.ondemand.com:443/cpi/v1/contentful", config);
    console.log("\u{1F680} ~ file: subscribe.ts ~ line 79 ~ consthandler:Handler= ~ response", response);
  } catch (error) {
    console.log("\u{1F680} ~ file: submission-created.ts ~ line 29 ~ consthandler:Handler= ~ error", error.response.data);
  }
  return {
    statusCode: 200,
    body: JSON.stringify({
